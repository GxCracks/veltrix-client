...
export type AppDeps = { config: AppConfig; store: VeltrixStore; };
...
export function createApp(deps: Partial<AppDeps> = {}): Express {
  ...
  app.get('/api/health', (_req, res) => { res.status(200).json({ ok: true, service: 'veltrix-api' }); });
  app.use('/api', createRoutes(store, config));
  app.use(notFound);
