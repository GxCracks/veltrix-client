...
export type WebSession={id:string;userId:string;tokenHash:string;csrfHash:string;createdAt:Date;expiresAt:Date;lastSeen:Date;revokedAt:Date|null};
...
export interface VeltrixStore{
 ...
 listCosmetics():Promise<Cosmetic[]>;getCosmetic(id:string):Promise<Cosmetic|null>;listOwnedCosmetics(uid:string):Promise<OwnedCosmetic[]>;grantCosmetic(uid:string,id:string,source:'purchase'|'admin_grant'|'beta_reward'|'promo'):Promise<boolean>;equipCosmetic(uid:string,id:string):Promise<'ok'|'not_owned'|'not_found'>;unequipCosmetic(uid:string,id:string):Promise<'ok'|'not_owned'|'not_found'>;
}
