...
export function createRoutes(store: VeltrixStore, config: AppConfig): Router {
  ...
  router.post('/client/session', limiter, async (req, res, next) => {
    try {
      ...
      await store.createClientSession({
        ...
        tokenHash: hashToken(token, config.tokenPepper),
        createdAt: now,
      ...
      res.status(201).json({
        ...
        user: {
          ...
          minecraftUuid: user.minecraftUuid,
          veltrixUserId: user.veltrixUserId,
        },
      });
    } catch (error) {
      next(error);
  ...
  router.post('/client/session/revoke', requireClient, async (req, res, next) => {
    try {
      await store.revokeClientSession(req.veltrixClientSession!.id, new Date());
      res.status(204).end();
    } catch (error) {
      next(error);
    }
  ...
  router.post('/web-login/request', limiter, async (req, res, next) => {
    try {
      ...
      const loginRequest: WebLoginRequest = {
        id: randomUUID(),
        verificationCodeHash: hashToken(code, config.tokenPepper),
        codeCiphertext: encryptShortSecret(code, config.sessionSecret),
        minecraftUuid: active.minecraftUuid,
    ...
    try {
      ...
      res.json({
        requests: requests.map(request => ({
          id: request.id,
          code: decryptShortSecret(request.codeCiphertext, config.sessionSecret),
  ...
  const decideLogin = (status: 'approved' | 'denied') => async (
    ...
    try {
      ...
      const updated = await store.setWebLoginStatus(String(req.params.id), session.userId, status, new Date());
  ...
  router.post('/web-login/:id/complete', limiter, async (req, res, next) => {
    try {
      ...
      await store.createWebSession({
        ...
        tokenHash: hashToken(token, config.tokenPepper),
        csrfHash: hashToken(csrfToken, config.tokenPepper),
        createdAt: now,
  ...
  router.get('/cosmetics', async (_req, res, next) => {
    try {
      res.json({ cosmetics: (await store.listCosmetics()).map(publicCosmetic) });
    } catch (error) {
      next(error);
    }
  });

  // Keep protected static paths before /cosmetics/:id so "owned" is never treated as an ID.
    ...
    try {
      ...
      res.json({
        cosmetics: cosmetics.map(cosmetic => ({
          ...
          purchasedAt: cosmetic.purchasedAt.toISOString(),
  ...
  const mutateCosmetic = (equip: boolean) => async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = equip
        ? await store.equipCosmetic(req.veltrixWebSession!.userId, input.data.cosmeticId)
        : await store.unequipCosmetic(req.veltrixWebSession!.userId, input.data.cosmeticId);
